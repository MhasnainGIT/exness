...
export function WithdrawalPage() {
  ...
  const [sourceAccount, setSourceAccount] = useState('Real Standard #12345678');
  ...
  return (
    <div className="space-y-8 max-w-[1100px] mx-auto pb-20">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[28px] font-black text-[#1a1b20] tracking-tight">Withdrawal</h1>
          <p className="text-[14px] text-[#5f6368] mt-1">Select a method to withdraw your funds</p>
        </div>
      </div>
      ...
      {step === 'methods' ? (
        <>
          ...
          {!isVerified && (
            <div className="bg-[#fef9e7] border border-[#fde047]/30 rounded-xl p-5 flex items-center justify-between shadow-sm animate-in fade-in zoom-in duration-300">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[13px] text-[#5f6368]">
                    Complete your profile verification to enable withdrawals and increase limits.
                  </p>
                </div>
              </div>
              <Button
          ...
          <section className="space-y-4 pt-4">
            <h2 className="text-[16px] font-bold text-[#1a1b20]">All withdrawal methods</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {METHODS.map((method) => (
                <Card 
                  ...
                  onClick={() => { if (!isVerified && method.id !== 'internal') { showToast('Verification required for this method', 'error'); return; } setSelectedMethod(method); setStep('amount'); }}
                >
              ...
              <Card className="border-gray-100 shadow-sm rounded-2xl overflow-hidden">
                <CardContent className="p-8 space-y-8">
                  <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                     ...
                     <div>
                        <span className="text-[11px] font-black text-[#8b8e94] uppercase tracking-widest">Withdrawal Method</span>
                        <h4 className="text-[14px] font-bold text-[#1a1b20]">{selectedMethod?.name}</h4>
                     </div>
                  </div>

                  <div className="space-y-4">
                    <label className="block text-[13px] font-bold text-[#5f6368]">From account</label>
                    <div className="flex items-center justify-between p-4 border border-gray-200 rounded-xl cursor-pointer hover:border-[#ffce00] transition-all">
                       <span className="text-[14px] font-bold text-[#1a1b20]">{sourceAccount}</span>
                       <ChevronDown className="h-4 w-4 text-[#8b8e94]" />
                    </div>
                    <div className="text-[12px] font-bold text-[#8b8e94]">Available: 1,452.20 USD</div>
                  </div>
                  ...
                  <button 
                    onClick={() => { showToast('Security verification code sent to email...', 'info'); setTimeout(() => { setStep('methods'); showToast('Withdrawal request processing!', 'success'); }, 2000); }}
                    className="w-full h-14 bg-[#1a1b20] hover:bg-black text-white font-black text-[15px] rounded-xl shadow-lg transition-all active:scale-[0.98] mt-4"
                  >
                    Withdraw
                  </button>
                </CardContent>
            ...
            <div className="space-y-4">
               <Card className="border-gray-100 shadow-sm rounded-2xl p-6">
                 ...
                 <div className="space-y-3">
                    ...
                    <div className="flex justify-between text-[13px]">
                       <span className="text-[#5f6368]">Conversion rate</span>
                       <span className="font-bold text-[#1a1b20]">1.0000</span>
                    </div>
                    <div className="h-px bg-gray-50 my-2" />
                    <div className="flex justify-between text-[15px] font-black">
                       <span className="text-[#1a1b20]">Total to receive</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
         <div className="bg-white border border-gray-200 rounded-xl p-5">
            <h4 className="text-[14px] font-bold text-[#1a1b20] mb-2 flex items-center gap-2">
               <AlertTriangle className="h-4 w-4 text-[#f59e0b]" /> Rules of withdrawal
            </h4>
            <ul className="text-[12px] text-[#5f6368] space-y-2 list-disc pl-4">
               <li>Withdrawals must be made to the same payment method used for deposit.</li>
               <li>Exness does not charge fees for withdrawals, but payment systems might.</li>
               <li>Processing time varies depending on the payment method selected.</li>
            </ul>
         </div>
      </div>
    </div>
